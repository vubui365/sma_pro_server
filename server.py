...
_ALLOWED_INTERVALS = frozenset(['1m', '5m', '15m', '30m', '1H', '1D'])
...
def resample_history(hist, tf):
    """Resample OHLCV history sang timeframe khác.
    Hỗ trợ: 1D, 1W, 1M, 1H, 4H, 2H, 30m, 15m, 5m
    """
    ...
    def flush():
        ...
        result.append({"o":bucket[0].get("o",bucket[0]["c"]),
                        ...
                        "d":bucket[-1].get("d") or bucket[-1].get("t","")})

    # Intraday bars: "d" có format "YYYY-MM-DD HH:MM" hoặc ISO
    _intraday_tfs = {'1H':60, '2H':120, '4H':240, '30m':30, '15m':15, '5m':5, '1m':1}
    if tf in _intraday_tfs:
        minutes = _intraday_tfs[tf]
        for bar in hist:
            raw_d = bar.get("d") or bar.get("t","")
            try:
                if 'T' in raw_d:
                    dt_obj = datetime.fromisoformat(raw_d[:16])
                elif ' ' in raw_d:
                    dt_obj = datetime.strptime(raw_d[:16], "%Y-%m-%d %H:%M")
                else:
                    # Daily bar → không resample xuống intraday được
                    return hist
                total_min = dt_obj.hour*60 + dt_obj.minute
                slot = (total_min // minutes) * minutes
                key = (dt_obj.date(), slot)
            except Exception:
                key = raw_d[:13]
...
def analyze_mtf(data, mode='daily'):
    """
    Multi-timeframe analysis.
    mode='daily'   → 1D / 1W / 1M (từ daily OHLCV, đang dùng cho VN/US)
    mode='intraday'→ 15m / 1H / 4H (từ intraday OHLCV, cần data intraday)
    """
    ...
    for tf in tfs:
        ...
        results[tf] = analyze(fake, tf)

    # Consensus
    totals = [results[tf]["total"] for tf in tfs]
...
def calc_black_litterman(user_id: int, views: list = None) -> dict:
    """
    Black-Litterman model: combines market equilibrium with investor views.
    views = [{'symbol': 'VCB', 'expected_return': 0.15, 'confidence': 0.6}, ...]
    """
    import math
    trades = trade_list(user_id, status='OPEN', limit=500)
    if len(trades) < 2:
        return {"error": "Cần ít nhất 2 vị thế"}
    symbol_returns = {}
    for t in trades:
        sym = t['symbol']
        bars = db_load_ohlcv(sym, 252)
        if len(bars) >= 60:
            cl = [b['c'] for b in bars if b['c'] > 0]
            rets = [(cl[i]-cl[i-1])/cl[i-1] for i in range(1, len(cl))]
            if rets: symbol_returns[sym] = rets
    syms = list(symbol_returns.keys())
    if len(syms) < 2:
        return {"error": "Chưa đủ lịch sử giá (cần 60+ phiên)"}
    n = len(syms)
    min_len = min(len(r) for r in symbol_returns.values())
    rm = {s: symbol_returns[s][-min_len:] for s in syms}
    # Market equilibrium returns (Pi = delta * Sigma * w_mkt)
    delta = 2.5  # risk aversion
    mu = [sum(rm[s])/min_len * 252 for s in syms]
    # Covariance matrix
    cov = [[sum((rm[syms[i]][k]-mu[i]/252)*(rm[syms[j]][k]-mu[j]/252)
                for k in range(min_len))/min_len * 252
            for j in range(n)] for i in range(n)]
    # Equal weight market portfolio
    w_mkt = [1.0/n] * n
    pi = [delta * sum(cov[i][j]*w_mkt[j] for j in range(n)) for i in range(n)]
    # Apply views if provided
    if views:
        tau = 0.05
        valid_views = [(v, syms.index(v['symbol'])) for v in (views or [])
                       if v.get('symbol') in syms]
        if valid_views:
            q = [v['expected_return'] for v, _ in valid_views]
            P = [[1.0 if k == idx else 0.0 for k in range(n)]
                 for _, idx in valid_views]
            omega_diag = [tau * cov[idx][idx] / max(v.get('confidence', 0.5), 0.01)
                         for v, idx in valid_views]
            # BL posterior: simplified closed-form
            tau_cov = [[tau * cov[i][j] for j in range(n)] for i in range(n)]
            try:
                bl_returns = list(pi)
                for vi, (view, idx) in enumerate(valid_views):
                    diff = q[vi] - sum(P[vi][k]*pi[k] for k in range(n))
                    omega_v = omega_diag[vi]
                    p_sigma_p = sum(P[vi][i]*tau_cov[i][j]*P[vi][j]
                                    for i in range(n) for j in range(n))
                    scale = diff / (p_sigma_p + omega_v) if (p_sigma_p + omega_v) > 0 else 0
                    for k in range(n):
                        bl_returns[k] += tau_cov[idx][k] * scale
                mu = bl_returns
            except Exception: pass
    # Optimal weights: maximize Sharpe
    import random; random.seed(42)
    best_sh = -999; best_w = None; best_r = 0; best_v = 0
    for _ in range(2000):
        raw = [random.expovariate(1) for _ in range(n)]
        tot = sum(raw); w = [x/tot for x in raw]
        port_r = sum(w[i]*mu[i] for i in range(n))
        port_v = math.sqrt(max(sum(w[i]*w[j]*cov[i][j]
                                   for i in range(n) for j in range(n)), 0))
        sh = (port_r - 0.045) / port_v if port_v > 0 else 0
        if sh > best_sh:
            best_sh = sh; best_w = w; best_r = port_r; best_v = port_v
    if not best_w: best_w = w_mkt
    return {
        'model': 'Black-Litterman',
        'optimal_weights': {syms[i]: round(best_w[i]*100, 1) for i in range(n)},
        'expected_return': round(best_r*100, 2),
        'expected_vol': round(best_v*100, 2),
        'sharpe_ratio': round(best_sh, 3),
        'symbols': syms,
        'equilibrium_returns': {syms[i]: round(pi[i]*100, 2) for i in range(n)},
        'views_applied': len([v for v in (views or []) if v.get('symbol') in syms]),
        'note': 'BL model kết hợp cân bằng thị trường + kỳ vọng nhà đầu tư',
    }

# ── RISK PARITY ──────────────────────────────────────────────
def calc_risk_parity(user_id: int) -> dict:
    ...
    syms = list(symbol_returns.keys())
    if len(syms) < 2:
        return {"error": "Chưa đủ dữ liệu"}
    n = len(syms); min_len = min(len(r) for r in symbol_returns.values())
    rm = {s: symbol_returns[s][-min_len:] for s in syms}
    # Inverse volatility weighting (approx risk parity)
    vols = [math.sqrt(sum((r - sum(rm[s])/min_len)**2
                          for r in rm[s])/min_len) * math.sqrt(252)
            for s in syms]
    inv_vols = [1.0/max(v, 0.001) for v in vols]
    total_inv = sum(inv_vols)
    weights = [iv/total_inv for iv in inv_vols]
    # Portfolio stats
    cov = [[sum((rm[syms[i]][k]-sum(rm[syms[i]])/min_len)*
                (rm[syms[j]][k]-sum(rm[syms[j]])/min_len)
                for k in range(min_len))/min_len * 252
            for j in range(n)] for i in range(n)]
    port_vol = math.sqrt(max(sum(weights[i]*weights[j]*cov[i][j]
                                  for i in range(n) for j in range(n)), 0))
    port_ret = sum(weights[i]*sum(rm[syms[i]])/min_len*252 for i in range(n))
    # Risk contributions
    marginal_rc = [sum(weights[j]*cov[i][j] for j in range(n)) for i in range(n)]
    risk_contrib = [weights[i]*marginal_rc[i]/max(port_vol**2, 0.0001)
                    for i in range(n)]
    return {
        'model': 'Risk Parity (Inverse Vol)',
        'weights': {syms[i]: round(weights[i]*100, 1) for i in range(n)},
        'risk_contributions': {syms[i]: round(risk_contrib[i]*100, 1) for i in range(n)},
        'portfolio_vol': round(port_vol*100, 2),
        'portfolio_return': round(port_ret*100, 2),
        'sharpe': round((port_ret - 0.045)/max(port_vol, 0.001), 3),
        'individual_vols': {syms[i]: round(vols[i]*100, 2) for i in range(n)},
    }

# ── CVaR (Conditional Value at Risk) ────────────────────────
def calc_cvar(user_id: int, confidence: float = 0.95, horizon_days: int = 10) -> dict:
    """CVaR = Expected Shortfall at confidence level."""
    import math
    trades = trade_list(user_id, status='OPEN', limit=500)
    if not trades:
        return {"error": "Không có vị thế đang mở"}
    port_returns = []
    total_val = 0.0
    weighted_rets = []
    for t in trades:
        ...
        cur = _ram_cache.get(sym, {}).get('data', {}).get('price', entry)
        pos_val = cur * qty; total_val += pos_val
        bars = db_load_ohlcv(sym, 252)
        if len(bars) >= 30:
            cl = [b['c'] for b in bars if b['c'] > 0]
            rets = [(cl[i]-cl[i-1])/cl[i-1] for i in range(1, len(cl))]
            if rets: weighted_rets.append((rets, pos_val))
    if not weighted_rets or total_val <= 0:
        return {"error": "Chưa đủ dữ liệu tính CVaR"}
    min_len = min(len(r) for r, _ in weighted_rets)
    port_rets = []
    for k in range(min_len):
        pr = sum(rets[k] * (val/total_val) for rets, val in weighted_rets)
        port_rets.append(pr)
    # Scale to horizon
    scaled = sorted([r * math.sqrt(horizon_days) for r in port_rets])
    cut = int((1-confidence) * len(scaled))
    if cut == 0: cut = 1
    var = -scaled[cut]
    cvar = -sum(scaled[:cut]) / cut if cut > 0 else var
    return {
        'VaR': round(var*100, 2),
        'CVaR': round(cvar*100, 2),
        'VaR_vnd': round(var * total_val, 0),
        'CVaR_vnd': round(cvar * total_val, 0),
        'confidence': confidence,
        'horizon_days': horizon_days,
        'portfolio_value': round(total_val, 0),
        'note': f"VaR {confidence*100:.0f}%: lỗ tối đa {var*100:.1f}% · CVaR: lỗ kỳ vọng khi vượt VaR",
    }
            ...
            try:
                ...
                for r in rows:
                        ...
                        with get_db() as c2:
                            ...
                            if not last or (datetime.now()-datetime.fromisoformat(last["created_at"])).total_seconds()>3600:
                                ...
                                if should_send:
                                    ...
                                    # ── FIX: Push live banner alert ──────────────────
                                    try:
                                        _sig   = a["signal"]
                                        _level = 'CRITICAL' if _sig in ('BUY','SELL') else 'WARNING'
                                        _icon  = '🟢' if 'BUY' in _sig else '🔴' if 'SELL' in _sig else '⚪'
                                        _vn    = mkt == 'VN'
                                        _fv    = f"{round(data['price']):,}đ" if _vn else f"${data['price']:.2f}"
                                        push_live_alert(
                                            sym, mkt, _level,
                                            f"{_icon} {_sig} — {sym} ({mkt})",
                                            f"Điểm: {a['total']:+d}/19 · Giá: {_fv}",
                                            action=f"Xem phân tích {sym}",
                                            price=data['price']
                                        )
                                    except Exception as _pe:
                                        log.debug("[banner] push %s: %s", sym, _pe)
                                else:
                                    print(f"[Alert] {sym} alert suppressed by gap filter: {gap_reason}")
                    ...
                    try:
                        ...
                        if _rv > 0:
                            _vn3 = (mkt == 'VN')
                            _pv3 = data.get('price', 0)
                            def _fp3(v, vn=_vn3): return (str(round(v))+',000d') if vn else ('$'+str(round(v,2)))
                            if _rv <= 25:
                                _ok, _ = smart_alert_cooldown('rsi_low_'+sym, 'RSI_LOW', ttl=14400)
                                if _ok:
                                    _body = 'RSI='+str(round(_rv,1))+' <=25 Oversold · Gia: '+str(round(_pv3))+('' if _vn3 else '')
                                    push_live_alert(sym, mkt, 'WARNING', '📉 RSI Cuc thap - '+sym, _body, action='Xem '+sym, price=_pv3)
                                    _tok = get_setting('tg_token','')
                                    if _tok:
                                        _mr = '📉 RSI CUC THAP - '+sym+' ('+mkt+')\nRSI: '+str(round(_rv,1))+' (<= 25 oversold)\nVung mua tiem nang - cho xac nhan dao chieu'
                                        threading.Thread(target=tg_send_to_subscribers,args=(sym,_mr),daemon=True).start()
                            elif _rv >= 75:
                                _ok, _ = smart_alert_cooldown('rsi_high_'+sym, 'RSI_HIGH', ttl=14400)
                                if _ok:
                                    _body = 'RSI='+str(round(_rv,1))+' >=75 Overbought · Gia: '+str(round(_pv3))
                                    push_live_alert(sym, mkt, 'WARNING', '📈 RSI Cuc cao - '+sym, _body, action='Xem '+sym, price=_pv3)
                                    _tok = get_setting('tg_token','')
                                    if _tok:
                                        _mr = '📈 RSI CUC CAO - '+sym+' ('+mkt+')\nRSI: '+str(round(_rv,1))+' (>= 75 overbought)\nVung ban tiem nang - can nhac chot loi'
                                        threading.Thread(target=tg_send_to_subscribers,args=(sym,_mr),daemon=True).start()
                    except Exception as _rse: log.debug('[RSI alert] %s: %s', sym, _rse)
                        ...
                        for lv,nm in [(tp1,'TP1 (2×ATR)'),(tp2,'TP2 (3×ATR)')]:
                            ...
                            if pnow >= lv*0.998:
                                ...
                                if not last_tp or (datetime.now()-datetime.fromisoformat(last_tp["created_at"])).total_seconds()>7200:
                                    ...
                                    # Push banner for TP
                                    try:
                                        push_live_alert(sym, mkt, 'CRITICAL', nm+' DAT - '+sym,
                                            'Gia dat '+nm+' · Xem xet chot loi', action='Chot loi/doi SL', price=pnow)
                                    except Exception: pass
                    ...
                    # Gan Ho tro/Khang cu alert (S1/R1 < 1.5%)
                    try:
                        _s1v = a.get('support_1', 0) or a.get('support', 0)
                        _r1v = a.get('resistance_1', 0) or a.get('resistance', 0)
                        _pnow4 = data.get('price', 0)
                        if _pnow4 > 0 and _s1v > 0:
                            _ds = abs(_pnow4 - _s1v) / _s1v * 100
                            if _ds < 1.5:
                                _ok, _ = smart_alert_cooldown('sr_sup_'+sym, 'SR_SUPPORT', ttl=14400)
                                if _ok:
                                    push_live_alert(sym, mkt, 'INFO', '○ Gan ho tro S1 - '+sym,
                                        'Gia cach S1 '+str(round(_ds,1))+'% · Canh mua/dat SL', action='Xem '+sym, price=_pnow4)
                        if _pnow4 > 0 and _r1v > 0:
                            _dr = abs(_r1v - _pnow4) / _r1v * 100
                            if _dr < 1.5:
                                _ok, _ = smart_alert_cooldown('sr_res_'+sym, 'SR_RESIST', ttl=14400)
                                if _ok:
                                    push_live_alert(sym, mkt, 'INFO', '○ Gan khang cu R1 - '+sym,
                                        'Gia cach R1 '+str(round(_dr,1))+'% · Can nhac chot/dat TP', action='Xem '+sym, price=_pnow4)
                    except Exception as _sre: log.debug('[SR alert] %s: %s', sym, _sre)
...
# ── SERVICE WORKER ──────────────────────────────────────────
SW_JS = """
const CACHE_NAME = 'sma-Pro-cache-v5-492840';
const STATIC_ASSETS = ['/manifest.json', '/sw.js'];  // Don't cache HTML pages
const API_CACHE_PATTERNS = ['/api/watchlist', '/api/settings', '/api/market_context'];
...
self.addEventListener('fetch', event => {
  ...
  if(event.request.destination === 'document' || reqUrl.endsWith('/') || 
     reqUrl.endsWith('.html')) {
    event.respondWith(fetch(event.request).catch(() => caches.match(event.request)));
    return;
  }
  // Bypass SW for these API routes completely
  const SW_BYPASS = ['/api/invest/', '/api/stock/', '/api/screener', '/api/backtest', '/api/ml_'];
  if(SW_BYPASS.some(p => reqUrl.includes(p))) {
    return;
  }
  if(reqUrl.includes('/api/')) {
    event.respondWith(
      fetch(event.request).catch(() =>
        new Response(JSON.stringify({error:'offline','msg':'Offline — vui lòng kiểm tra kết nối'}),
    ...
    if (isCacheable) {
      event.respondWith(
        fetch(event.request.clone()).then(resp => {
          ...
          JSON.stringify({error: 'Offline — dữ liệu cache', offline: true}),
          {headers: {'Content-Type': 'application/json'}}
        )))
      );
    }
    ...
    caches.match(event.request).then(cached => {
      ...
      return fetch(event.request).then(resp => {
        ...
        if (event.request.headers.get('accept').includes('text/html')) {
          return caches.match('/index_Pro.html').then(r=>r||caches.match('/index.html'));
        }
      });
    })
  );
...
# ── BCTC ĐẦY ĐỦ (FireAnt + TCBS) ─────────────────────────────────────────────

_BCTC_CACHE: dict = {}
_BCTC_TTL = 3600 * 6  # 6 giờ

def fetch_bctc_fireant(symbol: str) -> dict:
    """
    Lấy BCTC đầy đủ từ FireAnt API (public, không cần đăng nhập).
    Trả về: income_statement, balance_sheet, cashflow, ratios theo quý.
    """
    import urllib.request as _ur
    import json as _json

    cached = _BCTC_CACHE.get(symbol)
    if cached and time.time() - cached['ts'] < _BCTC_TTL:
        return cached['data']

    result = {
        'symbol': symbol, 'source': 'fireant',
        'income_quarters': [],   # 8 quý gần nhất
        'balance_quarters': [],
        'cashflow_quarters': [],
        'ratios_quarters': [],   # PE/PB/ROE/ROA trailing theo quý
    ...
    def fa_get(url):
        try:
            ...
            log.debug("[BCTC] FA request failed %s: %s", url, e)
            return None

    # 1. Income Statement quarterly
    try:
        url = f"https://restv2.fireant.vn/symbols/{symbol}/financial-reports?type=0&count=10"
        data = fa_get(url)
        if data and isinstance(data, list):
            for item in data[:8]:
                q = item.get('quarter',''); yr = item.get('year','')
                result['income_quarters'].append({
                    'period': f"{yr}Q{q}",
                    'year': yr, 'quarter': q,
                    'revenue':          _safe_float(item.get('netRevenue') or item.get('revenue')),
                    'gross_profit':     _safe_float(item.get('grossProfit')),
                    'operating_profit': _safe_float(item.get('operatingProfit') or item.get('ebit')),
                    'net_income':       _safe_float(item.get('netProfit') or item.get('profitAfterTax')),
                    'eps':              _safe_float(item.get('eps')),
                    'ebitda':           _safe_float(item.get('ebitda')),
                })
    except Exception as e:
        log.debug("[BCTC] income: %s", e)

    # 2. Balance Sheet quarterly
    try:
        url = f"https://restv2.fireant.vn/symbols/{symbol}/financial-reports?type=1&count=10"
        data = fa_get(url)
        if data and isinstance(data, list):
            for item in data[:8]:
                q = item.get('quarter',''); yr = item.get('year','')
                result['balance_quarters'].append({
                    'period': f"{yr}Q{q}",
                    'total_assets':     _safe_float(item.get('totalAssets')),
                    'total_equity':     _safe_float(item.get('equity') or item.get('totalEquity')),
                    'total_debt':       _safe_float(item.get('debt') or item.get('totalDebt')),
                    'cash':             _safe_float(item.get('cash') or item.get('cashAndEquivalents')),
                    'short_term_debt':  _safe_float(item.get('shortTermDebt')),
                    'long_term_debt':   _safe_float(item.get('longTermDebt')),
                    'current_assets':   _safe_float(item.get('currentAssets')),
                    'current_liabilities': _safe_float(item.get('currentLiabilities')),
                })
    except Exception as e:
        log.debug("[BCTC] balance: %s", e)

    # 3. Cashflow quarterly
    try:
        url = f"https://restv2.fireant.vn/symbols/{symbol}/financial-reports?type=2&count=10"
        data = fa_get(url)
        if data and isinstance(data, list):
            for item in data[:8]:
                q = item.get('quarter',''); yr = item.get('year','')
                result['cashflow_quarters'].append({
                    'period': f"{yr}Q{q}",
                    'operating_cf':  _safe_float(item.get('operatingCashFlow') or item.get('cashFromOperations')),
                    'investing_cf':  _safe_float(item.get('investingCashFlow') or item.get('cashFromInvesting')),
                    'financing_cf':  _safe_float(item.get('financingCashFlow') or item.get('cashFromFinancing')),
                    'free_cf':       _safe_float(item.get('freeCashFlow')),
                    'capex':         _safe_float(item.get('capex')),
                })
    except Exception as e:
        log.debug("[BCTC] cashflow: %s", e)

    # 4. Key Ratios trailing (FA overview)
    try:
        url = f"https://restv2.fireant.vn/symbols/{symbol}/overview"
        data = fa_get(url)
        if data:
            fd = data.get('financialData', data)
            result['summary'] = {
                'pe_ttm':       _safe_float(fd.get('pe') or fd.get('priceToEarning')),
                'pb':           _safe_float(fd.get('pb') or fd.get('priceToBook')),
                'ps':           _safe_float(fd.get('ps') or fd.get('priceToSales')),
                'roe_ttm':      _safe_float(fd.get('roe') or fd.get('returnOnEquity')),
                'roa_ttm':      _safe_float(fd.get('roa') or fd.get('returnOnAsset')),
                'eps_ttm':      _safe_float(fd.get('eps') or fd.get('earningPerShare')),
                'bvps':         _safe_float(fd.get('bvps') or fd.get('bookValuePerShare')),
                'dividend_yield': _safe_float(fd.get('dividendYield') or fd.get('yield')),
                'debt_to_equity': _safe_float(fd.get('debtToEquity') or fd.get('leverage')),
                'current_ratio':  _safe_float(fd.get('currentRatio')),
                'market_cap':     _safe_float(fd.get('marketCap')),
                'shares_outstanding': _safe_float(fd.get('sharesOutstanding') or fd.get('issuedShare')),
                'beta':           _safe_float(fd.get('beta')),
                '52w_high':       _safe_float(fd.get('week52High')),
                '52w_low':        _safe_float(fd.get('week52Low')),
            }
    except Exception as e:
        log.debug("[BCTC] overview: %s", e)

    # 5. Tính growth metrics từ quarters
    if len(result['income_quarters']) >= 5:
        try:
            q = result['income_quarters']
            # YoY revenue growth (Q0 vs Q4)
            rev0 = q[0].get('revenue') or 0
            rev4 = q[4].get('revenue') or 0
            if rev4 and rev0:
                result['summary']['revenue_growth_yoy'] = round((rev0/rev4 - 1)*100, 1)
            ni0 = q[0].get('net_income') or 0
            ni4 = q[4].get('net_income') or 0
            if ni4 and ni0:
                result['summary']['profit_growth_yoy'] = round((ni0/ni4 - 1)*100, 1)
            # TTM revenue & income
            result['summary']['revenue_ttm'] = sum(q[i].get('revenue') or 0 for i in range(4))
            result['summary']['net_income_ttm'] = sum(q[i].get('net_income') or 0 for i in range(4))
            # Net margin TTM
            if result['summary']['revenue_ttm']:
                result['summary']['net_margin'] = round(
                    result['summary']['net_income_ttm'] / result['summary']['revenue_ttm'] * 100, 1)
        except Exception as e:
            log.debug("[BCTC] growth calc: %s", e)

    _BCTC_CACHE[symbol] = {'ts': time.time(), 'data': result}
    return result


def fetch_bctc_full(symbol: str, market: str = 'VN') -> dict:
    """
    Kết hợp BCTC: FireAnt (primary) + vnstock (fallback) + TCBS KQKD.
    """
    if market != 'VN':
        # US: dùng yfinance financials
        return _fetch_bctc_us(symbol)

    # Lấy FireAnt trước
    fa = fetch_bctc_fireant(symbol)

    # Merge với fetch_fundamental_vn (ratios từ vnstock)
    try:
        vn = fetch_fundamental_vn(symbol)
        if fa.get('summary'):
            # Ưu tiên FireAnt, fallback vnstock cho các field thiếu
            for k in ['pe_ttm','pb','roe_ttm','roa_ttm','eps_ttm','debt_to_equity','dividend_yield']:
                vn_key = k.replace('_ttm','').replace('pe_ttm','pe').replace('roe_ttm','roe').replace('roa_ttm','roa').replace('eps_ttm','eps')
                if fa['summary'].get(k) is None and vn.get(vn_key) is not None:
                    fa['summary'][k] = vn[vn_key]
            # Lấy income_statements từ vnstock nếu FA trống
            if not fa['income_quarters'] and vn.get('income_statements'):
                fa['income_quarters'] = vn['income_statements']
    except Exception as e:
        log.debug("[BCTC] vnstock fallback: %s", e)

    # Tính fundamental_score
    try:
        s = fa.get('summary', {})
        fd_for_score = {
            'pe': s.get('pe_ttm'), 'pb': s.get('pb'),
            'roe': s.get('roe_ttm'), 'roa': s.get('roa_ttm'),
            'revenue_growth_yoy': s.get('revenue_growth_yoy'),
            'profit_growth_yoy': s.get('profit_growth_yoy'),
            'debt_to_equity': s.get('debt_to_equity'),
            'net_margin': s.get('net_margin'),
        }
        fa['score'] = fundamental_score(fd_for_score)
        fa['dcf']   = calc_dcf(symbol, 'VN', fd_for_score)
    except Exception as e:
        log.debug("[BCTC] score/dcf: %s", e)

    return fa


def _fetch_bctc_us(symbol: str) -> dict:
    """BCTC cho US stocks qua yfinance."""
    result = {'symbol': symbol, 'source': 'yfinance', 'income_quarters': [],
              'balance_quarters': [], 'cashflow_quarters': [], 'summary': {}, 'error': None}
    try:
        import yfinance as yf
        t = yf.Ticker(symbol)
        info = t.info or {}
        result['summary'] = {
            ...
            'roa_ttm':        _safe_float(info.get('returnOnAssets')),
            ...
            'profit_growth_yoy':  _safe_float(info.get('earningsGrowth')),
        ...
        try:
            ...
            if qi is not None and not qi.empty:
                for col in qi.columns[:8]:
                    result['income_quarters'].append({
                        'period': str(col)[:7],
                        'revenue':      _safe_float(qi.get('Total Revenue', {}).get(col)),
                        'gross_profit': _safe_float(qi.get('Gross Profit', {}).get(col)),
                        'net_income':   _safe_float(qi.get('Net Income', {}).get(col)),
                        'ebitda':       _safe_float(qi.get('EBITDA', {}).get(col)),
                    })
        except Exception: pass
    except Exception as e:
        result['error'] = str(e)
...
# ── INTRADAY 1H/4H FETCH ─────────────────────────────────────────────────────

_INTRADAY_CACHE: dict = {}
_INTRADAY_TTL = 300  # 5 phút

def fetch_intraday_vn(symbol: str, interval: str = '1H', limit: int = 200) -> list:
    """
    Lấy dữ liệu intraday VN từ TCBS API.
    interval: '1m','5m','15m','30m','1H','4H'
    Trả về: list of {d, o, h, l, c, v}
    """
    cache_key = f"{symbol}_{interval}"
    cached = _INTRADAY_CACHE.get(cache_key)
    if cached and time.time() - cached['ts'] < _INTRADAY_TTL:
        return cached['data']

    # Map interval sang TCBS resolution
    _interval_map = {
        '1m': '1', '5m': '5', '15m': '15', '30m': '30',
        '1H': '60', '4H': '240', '1D': 'D'
    }
    resolution = _interval_map.get(interval, '60')
    ...
    try:
        # TCBS chart API
        to_ts   = int(time.time())
        from_ts = to_ts - limit * int(resolution) * 60 if resolution.isdigit() else to_ts - 365*86400
        ...
        raw = data.get('data', data.get('bars', []))
        if not raw:
            # Thử format khác
            if 't' in data and 'o' in data:
                ...
                for i, ts in enumerate(ts_list):
                    ...
                    bars.append({
                        'd': dt_str,
                        'o': float(data['o'][i]) * 1000 if data['o'][i] < 1000 else float(data['o'][i]),
                        'h': float(data['h'][i]) * 1000 if data['h'][i] < 1000 else float(data['h'][i]),
                        'l': float(data['l'][i]) * 1000 if data['l'][i] < 1000 else float(data['l'][i]),
                        'c': float(data['c'][i]) * 1000 if data['c'][i] < 1000 else float(data['c'][i]),
                        'v': float(data['v'][i]) if i < len(data.get('v', [])) else 0,
                    })
        else:
            for item in raw:
                ts = item.get('time') or item.get('tradingDate') or item.get('t')
                if isinstance(ts, (int, float)):
                    dt_str = datetime.fromtimestamp(ts).strftime('%Y-%m-%d %H:%M')
                else:
                    dt_str = str(ts)[:16]
                c = float(item.get('close', item.get('c', 0)))
                # TCBS giá đơn vị nghìn đồng
                if c < 1000: c *= 1000
                o = float(item.get('open',  item.get('o', c)))
                h = float(item.get('high',  item.get('h', c)))
                l = float(item.get('low',   item.get('l', c)))
                if o < 1000: o *= 1000
                if h < 1000: h *= 1000
                if l < 1000: l *= 1000
                bars.append({'d': dt_str, 'o': o, 'h': h, 'l': l, 'c': c,
                             'v': float(item.get('volume', item.get('v', 0)))})
    except Exception as e:
        log.debug("[Intraday] TCBS %s %s: %s", symbol, interval, e)

    # Fallback: dùng OHLCV daily đã có nếu không lấy được
    if not bars:
        try:
            daily = db_load_ohlcv(symbol, 500)
            if daily:
                # Resample daily → intraday (synthetic, kém chính xác nhưng vẫn có)
                bars = resample_history(daily, interval) or daily[-limit:]
        except Exception: pass

    if bars:
        bars.sort(key=lambda x: x.get('d',''))
        _INTRADAY_CACHE[cache_key] = {'ts': time.time(), 'data': bars[-limit:]}
        return bars[-limit:]
    return []


def fetch_intraday_us(symbol: str, interval: str = '1H', limit: int = 200) -> list:
    """Intraday US qua yfinance."""
    cache_key = f"{symbol}_{interval}_us"
    cached = _INTRADAY_CACHE.get(cache_key)
    if cached and time.time() - cached['ts'] < _INTRADAY_TTL:
        return cached['data']
    _yf_map = {'1m':'1m','5m':'5m','15m':'15m','30m':'30m','1H':'1h','4H':'1h','1D':'1d'}
    period_map = {'1m':'1d','5m':'5d','15m':'60d','30m':'60d','1H':'730d','4H':'730d','1D':'max'}
    interval_yf = _yf_map.get(interval, '1h')
    period_yf   = period_map.get(interval, '60d')
    bars = []
    try:
        import yfinance as yf
        df = yf.Ticker(symbol).history(period=period_yf, interval=interval_yf)
        ...
        log.debug("[Intraday] US %s %s: %s", symbol, interval, e)
    if bars:
        _INTRADAY_CACHE[cache_key] = {'ts': time.time(), 'data': bars[-limit:]}
    return bars[-limit:]


# ── PATTERN ALERT SCANNER ────────────────────────────────────────────────────

def detect_candle_patterns(bars: list) -> list:
    if not bars or len(bars) < 3: return []
    patterns = []
    b = bars; n = len(b)
    c0=b[-1]; c1=b[-2] if n>=2 else c0
    c2=b[-3] if n>=3 else c1; c3=b[-4] if n>=4 else c2

    def body(x):  return abs(x['c']-x['o'])
    def upper(x): return x['h']-max(x['c'],x['o'])
    def lower(x): return min(x['c'],x['o'])-x['l']
    def bull(x):  return x['c']>=x['o']
    def bear(x):  return x['c']<x['o']
    def rng(x):   return x['h']-x['l']

    body0=body(c0); rng0=rng(c0); body1=body(c1); rng1=rng(c1)

    # Pin Bar
    if rng0>0:
        lr=lower(c0)/rng0; ur=upper(c0)/rng0; br=body0/rng0
        if lr>=0.6 and ur<=0.1 and br<=0.35:
            patterns.append({'name':'Hammer','type':'BULLISH','strength':'STRONG' if lr>=0.7 else 'MODERATE','bar_index':0,'description':'Nến búa — đuôi dưới dài, báo hiệu đảo chiều tăng'})
        if ur>=0.6 and lr<=0.1 and br<=0.35:
            patterns.append({'name':'Shooting Star','type':'BEARISH','strength':'STRONG' if ur>=0.7 else 'MODERATE','bar_index':0,'description':'Sao băng — đuôi trên dài, báo hiệu đảo chiều giảm'})
        if br<=0.08:
            patterns.append({'name':'Doji','type':'NEUTRAL','strength':'MODERATE','bar_index':0,'description':'Doji — thị trường do dự, chờ xác nhận hướng'})
        if ur>=0.55 and lr<=0.15 and br<=0.35 and bear(c1):
            patterns.append({'name':'Inverted Hammer','type':'BULLISH','strength':'MODERATE','bar_index':0,'description':'Búa ngược — sau giảm, có thể đảo chiều tăng'})

    # Engulfing
    if body0>0 and body1>0:
        if bear(c1) and bull(c0) and c0['o']<=c1['c'] and c0['c']>=c1['o'] and body0>=body1*1.1:
            patterns.append({'name':'Bullish Engulfing','type':'BULLISH','strength':'STRONG' if body0>=body1*1.5 else 'MODERATE','bar_index':0,'description':'Nến nhấn chìm tăng — mua mạnh, đảo chiều lên'})
        if bull(c1) and bear(c0) and c0['o']>=c1['c'] and c0['c']<=c1['o'] and body0>=body1*1.1:
            patterns.append({'name':'Bearish Engulfing','type':'BEARISH','strength':'STRONG' if body0>=body1*1.5 else 'MODERATE','bar_index':0,'description':'Nến nhấn chìm giảm — bán mạnh, đảo chiều xuống'})

    # Harami
    if body0>0 and body1>0:
        if bear(c1) and bull(c0) and c0['o']>=c1['c'] and c0['c']<=c1['o'] and body0<=body1*0.6:
            patterns.append({'name':'Bullish Harami','type':'BULLISH','strength':'MODERATE','bar_index':0,'description':'Harami tăng — nến nhỏ trong nến lớn giảm'})
        if bull(c1) and bear(c0) and c0['o']<=c1['c'] and c0['c']>=c1['o'] and body0<=body1*0.6:
            patterns.append({'name':'Bearish Harami','type':'BEARISH','strength':'MODERATE','bar_index':0,'description':'Harami giảm — nến nhỏ trong nến lớn tăng'})

    # Morning/Evening Star (3 nến)
    if n>=3:
        if (bear(c2) and body(c2)>rng(c2)*0.5 and body(c1)<body(c2)*0.4 and
            bull(c0) and body(c0)>body(c2)*0.5 and c0['c']>(c2['o']+c2['c'])/2):
            patterns.append({'name':'Morning Star','type':'BULLISH','strength':'STRONG','bar_index':0,'description':'Sao mai — đảo chiều tăng mạnh sau downtrend'})
        if (bull(c2) and body(c2)>rng(c2)*0.5 and body(c1)<body(c2)*0.4 and
            bear(c0) and body(c0)>body(c2)*0.5 and c0['c']<(c2['o']+c2['c'])/2):
            patterns.append({'name':'Evening Star','type':'BEARISH','strength':'STRONG','bar_index':0,'description':'Sao hôm — đảo chiều giảm sau uptrend'})

    # Three White Soldiers / Three Black Crows
    if n>=3:
        if bull(c0) and bull(c1) and bull(c2) and c0['c']>c1['c']>c2['c'] and body(c0)>rng(c0)*0.6:
            patterns.append({'name':'Three White Soldiers','type':'BULLISH','strength':'STRONG','bar_index':0,'description':'Ba chiến binh trắng — uptrend mạnh'})
        if bear(c0) and bear(c1) and bear(c2) and c0['c']<c1['c']<c2['c'] and body(c0)>rng(c0)*0.6:
            patterns.append({'name':'Three Black Crows','type':'BEARISH','strength':'STRONG','bar_index':0,'description':'Ba con quạ đen — downtrend mạnh'})

    # Marubozu
    if rng0>0 and body0/rng0>=0.92:
        ptype='BULLISH' if bull(c0) else 'BEARISH'
        patterns.append({'name':('Bullish' if bull(c0) else 'Bearish')+' Marubozu','type':ptype,'strength':'STRONG','bar_index':0,'description':'Marubozu — momentum ' +('mua' if bull(c0) else 'bán')+' rất mạnh'})

    # Inside Bar
    if c0['h']<=c1['h'] and c0['l']>=c1['l'] and body0<body1*0.8:
        patterns.append({'name':'Inside Bar','type':'BULLISH' if bull(c1) else 'BEARISH','strength':'MODERATE','bar_index':0,'description':'Inside bar — tích lũy, chờ breakout'})

    # Dark Cloud Cover / Piercing Line
    if n>=2:
        mid1=(c1['o']+c1['c'])/2
        if bull(c1) and bear(c0) and c0['o']>=c1['c']*0.995 and c0['c']<=mid1 and c0['c']>=c1['o']:
            patterns.append({'name':'Dark Cloud Cover','type':'BEARISH','strength':'STRONG','bar_index':0,'description':'Đám mây đen — mở cao đóng dưới giữa, bearish'})
        if bear(c1) and bull(c0) and c0['o']<=c1['c']*1.005 and c0['c']>=mid1 and c0['c']<=c1['o']:
            patterns.append({'name':'Piercing Line','type':'BULLISH','strength':'STRONG','bar_index':0,'description':'Đường xuyên thấu — mở thấp đóng trên giữa, bullish'})
    return patterns


def scan_patterns_watchlist(symbols_markets: list, min_strength: str = 'MODERATE') -> list:
    strength_order = {'STRONG':0,'MODERATE':1,'WEAK':2}
    min_idx = strength_order.get(min_strength,1)
    results = []
    for item in symbols_markets:
        sym=item.get('symbol','').upper(); mkt=item.get('market','VN')
        if not sym: continue
        try:
            bars = db_load_ohlcv(sym, 20)
            if not bars or len(bars)<3: continue
            price = bars[-1].get('c',0)
            for p in detect_candle_patterns(bars):
                if strength_order.get(p['strength'],99) <= min_idx:
                    results.append({'symbol':sym,'market':mkt,'price':price,
                        'pattern':p['name'],'type':p['type'],'strength':p['strength'],
                        'description':p['description'],'detected_at':datetime.now().strftime('%H:%M %d/%m')})
        except Exception as e: log.debug("[PatternScan] %s: %s", sym, e)
    results.sort(key=lambda x:(strength_order.get(x['strength'],9),0 if x['type']=='BULLISH' else 1))
    return results


def _check_pattern_alerts(sym: str, mkt: str, bars: list):
    if not bars or len(bars)<3: return
    try:
        patterns = [p for p in detect_candle_patterns(bars) if p['strength']=='STRONG']
        if not patterns: return
        price=bars[-1].get('c',0)
        pub=get_setting('public_url',f'http://localhost:{PORT}')
        now=datetime.now().strftime('%d/%m/%Y %H:%M')
        fp=f"{round(price):,}d" if mkt=='VN' else f"${price:.2f}"
        sep="━━━━━━━━━━━━━━━━━━━━"
        for p in patterns[:2]:
            icon='🟢' if p['type']=='BULLISH' else '🔴' if p['type']=='BEARISH' else '⚪'
            VER=globals().get('VERSION','Pro')
            msg=(f"📊 <b>SMA v{VER}</b> · Pattern Alert\n"
                 f"{icon} <b>{sym} ({mkt})</b> — {p['name']}\n"
                 f"{sep}\n📌 {p['description']}\n"
                 f"💪 Độ mạnh: <b>{p['strength']}</b>\n"
                 f"💰 Giá: <b>{fp}</b>\n🕐 {now}\n{sep}\n🔗 {pub}")
            cd_key=f"pattern_{sym}_{p['name']}"
            if _alert_cd.get(cd_key,0)<time.time():
                tg_send(msg)
                _alert_cd[cd_key]=time.time()+3600
    except Exception as e: log.debug("[PatternAlert] %s: %s", sym, e)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Efficient Frontier
        if path == "/api/portfolio/bl":
            u = self.require_auth(); 
            if not u: return
            try:
                bdata = json.loads(self.rfile.read(int(self.headers.get('Content-Length',0)))) if self.headers.get('Content-Length') else {}
            except: bdata = {}
            views = bdata.get('views', []) if bdata else []
            self.send_json(calc_black_litterman(u['id'], views)); return
        if path == "/api/portfolio/risk_parity":
            u = self.require_auth();
            if not u: return
            self.send_json(calc_risk_parity(u['id'])); return
        if path == "/api/portfolio/cvar":
            u = self.require_auth();
            if not u: return
            self.send_json(calc_cvar(u['id'])); return
        ...
        if path == "/api/drawings":
            ...
            sym = qs.get("symbol", [""])[0].upper()
            if not sym:
                self.send_json({"drawings": []}); return
            with get_db() as c:
        ...
        if path.startswith("/api/bctc/"):
            ...
            mkt2 = qs.get("market", ["VN"])[0].upper()
            try:
                result = fetch_bctc_full(sym2, mkt2)
                self.send_json(result)
            except Exception as e:
                self.send_json({"error": str(e)}, 500)
            return
        ...
        # ── INTRADAY 1H/4H ───────────────────────────────────────────────────────
        if path.startswith("/api/intraday_ohlcv/"):
            sym2     = path.split("/")[-1].upper()
            mkt2     = qs.get("market", ["VN"])[0].upper()
            interval = qs.get("interval", ["1H"])[0]
            ...
            try:
                ...
                mtf_intraday = {}
                if bars and len(bars) >= 20:
                    fake = {"history": bars, "price": bars[-1]["c"], "market": mkt2}
                    try:
                        mtf_intraday = analyze_mtf(fake, mode="intraday")
                    except Exception: pass
                self.send_json({"symbol": sym2, "market": mkt2, "interval": interval,
                                "bars": bars, "mtf": mtf_intraday, "count": len(bars)})
            except Exception as e:
                self.send_json({"error": str(e)}, 500)
            return

        # ── PATTERN SCAN ─────────────────────────────────────────────────────────
        if path == "/api/pattern_scan":
            mkt2     = qs.get("market", ["ALL"])[0].upper()
            strength = qs.get("strength", ["MODERATE"])[0].upper()
            try:
                import concurrent.futures as _cf
                with get_db() as c:
                    if mkt2 == "ALL":
                        ...
                        rows = c.execute("SELECT DISTINCT symbol, market FROM user_watchlist WHERE market=?", (mkt2,)).fetchall()
                symbols = [{"symbol": r[0], "market": r[1]} for r in rows]
                if not symbols:
                    self.send_json({"patterns": [], "count": 0}); return
                with _cf.ThreadPoolExecutor(max_workers=4) as ex:
                    result = ex.submit(scan_patterns_watchlist, symbols, strength).result(timeout=30)
                self.send_json({"patterns": result, "count": len(result),
                                "scanned": len(symbols), "strength_filter": strength})
            except Exception as e:
                self.send_json({"error": str(e)}, 500)
            return

        if path.startswith("/api/pattern_detect/"):
            sym2 = path.split("/")[-1].upper()
            mkt2 = qs.get("market", ["VN"])[0].upper()
            try:
                bars = db_load_ohlcv(sym2, 20)
                if not bars:
                    self.send_json({"patterns": [], "error": "No data"})
                    return
                patterns = detect_candle_patterns(bars)
                self.send_json({"symbol": sym2, "market": mkt2,
                                "patterns": patterns, "price": bars[-1].get("c", 0)})
            except Exception as e:
                self.send_json({"error": str(e)}, 500)
            return
